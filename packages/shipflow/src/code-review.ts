import type { CodeReviewFinding, FeatureRequestAssessment } from "./types";
  ...
  if (!openAiApiKey) {
    return {
      ...
      findings: [{
        ...
        message: "Set OPENAI_API_KEY in your environment",
      }],
    };
  }

  const systemPrompt = `You are Jal QA Agent for ${shipflow.productName}. Review a GitHub pull request against the product requirements.
${shipflow.productContext}

Evaluate whether the implementation satisfies the PRD for this product domain — not just syntax.
...
- reject if fundamentally wrong direction`;
  ...
  const result = await openAiJson<AiCodeReviewResult>(openAiApiKey, systemPrompt, userContent);
  if (!result?.verdict) {
    return {
      verdict: "fix_needed",
